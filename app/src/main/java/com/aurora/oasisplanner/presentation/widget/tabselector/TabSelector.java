package com.aurora.oasisplanner.presentation.widget.tabselector;
...
public class TabSelector extends LinearLayout{
    private TabSelectorBinding binding;
    ...
    private void init(Context context, AttributeSet attrs) {
        binding = TabSelectorBinding.inflate(LayoutInflater.from(context), this, true);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TabSelector);
        background = a.getColor(R.styleable.TabSelector_bgColor, Color.parseColor("#6200EE"));
        a.recycle();

        binding.selectContent.getBackground().setColorFilter(background, PorterDuff.Mode.SRC_IN);
    ...
    public void onClick(int id) {
        try {
            ...
            ((TextView) binding.frontTabs.getChildAt(selectedId)).setTextColor(Color.GRAY);
            ((TextView) binding.frontTabs.getChildAt(id)).setTextColor(Color.WHITE);
            selectedId = id;
            if (ocl != null)
    ...
    public TabSelectorBinding getBinding() {
        return binding;
    }

    public interface OnChangeListener {
        void run(int i, List<MenuItem> menu, TabSelectorBinding binding);
    }
